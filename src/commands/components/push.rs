...
use crate::api_client::{auth::Config, ErrorExt, ResultExt};
use inquire::{Confirm, Editor, Select};

use crate::api_client::{types as api_types, Client};
...
pub async fn run(opts: Options) -> Result<()> {
    ...
    let client = crate::api_client::new().credentials(&creds).connect();
    ...
    let (do_update, component) = match client
        ...
        .await
    {
        Err(crate::api_client::Error::ErrorResponse(err))
            if err.error.type_
                == crate::api_client::types::ErrorResponseErrorType::NotFoundError =>
        {
            tracing::info!(
...
async fn update_component(
    ...
    use crate::api_client::ResultExt;
