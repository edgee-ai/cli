...
pub async fn run(_opts: Options) -> anyhow::Result<()> {
    ...
    let subcategories = match component_category.value {
        crate::api_client::types::ComponentCreateInputCategory::DataCollection => {
            DATA_COLLECTION_SUBCATEGORY_OPTIONS.to_vec()
        }
        crate::api_client::types::ComponentCreateInputCategory::EdgeFunction => {
        ...
        component: Component {
            ...
            settings: indexmap::indexmap! {
                "example".to_string() => Setting {
                    ...
                    type_: crate::api_client::types::ConfigurationFieldType::String,
                    secret: Some(false),
                },
            },
            build: Build {
